...
To address these severe geometric distortions, this project introduces a novel enhancement pipeline that explicitly extracts and combines rigid camera poses and estimated depth. Instead of treating \mvdp{} as a black-box point cloud generator, our method extracts the robust internal camera poses and coordinate system implicitly solved by the \mvdp{} multi-view decoder. We then combine these highly accurate rigid poses with auxiliary metric depth maps---produced by a fine-tuned monocular depth estimator---to perform a mathematically rigorous "source-ray correction."

The main motivation is practical deployment. The system requires only sparse RGB views at deployment time, satisfying the RGB-only contract. The extracted \mvdp{} poses provide structural rigidity, while the fine-tuned depth estimator provides per-pixel geometric priors that correct occlusions and scale ambiguities.

The main contributions of this report are:
\begin{enumerate}[leftmargin=1.2cm]
    \item It proposes a novel pipeline that extracts internal camera poses from the RGB-only \mvdp{} backbone and combines them with a fine-tuned monocular depth estimator.
    \item It describes a ray-based estimated-depth correction module that rigorously refines the candidate geometry and resolves sparse-view structural distortions.
...
\begin{figure}[H]
    ...
    \caption{Detailed overview of the \mvdp{} cross-view reasoning process.}
...
\begin{figure}[H]
    ...
    \caption{Overview of the dataset environment showing the diversity of indoor scenes and the dense 3D ground truth structure.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.24\textwidth}
        ...
        \caption{View 4}
    \end{subfigure}
    \caption{Example of the sparse RGB input images used for 3D reconstruction. The system must infer multi-view geometry and depth solely from these limited appearance viewpoints.}
...
The proposed correction module then combines the rigid camera poses extracted from the \mvdp{} output with the fine-tuned metric depth anchors. A geometric correction algorithm is applied to pull structurally distorted candidate points onto the physical surfaces defined by the metric depth rays.

All inferences and fine-tuning processes are executed in a controlled Kaggle notebook environment equipped with NVIDIA GPUs. The system ultimately outputs reconstructed 3D point clouds mapped to the correct global coordinate frame for downstream 3D visualization and quantitative evaluation.
