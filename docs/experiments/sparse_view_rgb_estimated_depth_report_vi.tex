...
\vspace{1.0cm}
{\Large Báo cáo Đồ án môn Thị giác Máy tính\\}
\vspace{1.2cm}
...
Thí nghiệm tái dựng chính đánh giá liệu hiệu chỉnh bằng độ sâu ước lượng có cải thiện so với RGB-only \mvdp{} hay không. Bảng~\ref{tab:recon} tổng hợp các kết quả đã hoàn tất từ lịch sử chạy thí nghiệm. Các số này dùng controlled sparse-view protocol của project, không phải official full ScanNet hoặc ScanNet++ benchmark.
...
Backprojection trực tiếp từ độ sâu ước lượng & 0.1627 & 0.1641 & 0.2029 & 0.1834 & 0.1894 & 0.3268 \\
Hiệu chỉnh bằng độ sâu pretrained & 0.2240 & 0.2055 & 0.1707 & 0.1343 & 0.1465 & 0.4295 \\
Hiệu chỉnh bằng độ sâu đo thật RGB-D & 0.1272 & 0.1385 & 0.3622 & 0.2283 & 0.2745 & 0.2656 \\
Hiệu chỉnh bằng độ sâu ước lượng đã tinh chỉnh (Ours) & \multicolumn{6}{c}{Run 38 đang chạy; chưa dùng số khi output chưa hoàn tất} \\
\bottomrule
\end{tabular}%
}
\caption{So sánh tái dựng 3D trên test groups của các run đã hoàn tất. Hàng RGB-D đo thật là mốc tham chiếu/upper reference trong controlled setting, không phải setting triển khai RGB-only. Hàng Ours phụ thuộc Run 38 với depth estimator đã fine-tune và chỉ nên điền sau khi Kaggle output hoàn tất.}
\label{tab:recon}
\end{table}

Câu hỏi chính là liệu phương pháp đề xuất có cải thiện so với RGB-only \mvdp{} hay không. Nếu hiệu chỉnh bằng độ sâu ước lượng đã tinh chỉnh đạt F-score cao hơn và Chamfer distance thấp hơn so với baseline RGB-only, thì bộ ước lượng độ sâu đã cung cấp hiệu chỉnh hình học hữu ích trong khi vẫn giữ ràng buộc suy luận chỉ dùng RGB.
...
Công việc tiếp theo nên hoàn thiện thí nghiệm fine-tuned depth, đánh giá toàn bộ các nhóm sparse-view có sẵn, thêm cơ chế confidence-aware correction và kiểm thử phương pháp trên các tập ảnh RGB thực tế đa dạng hơn. Mục tiêu dài hạn là một hệ thống tái dựng có thể triển khai được, có khả năng tạo hình học cảnh 3D hữu ích từ chỉ một vài ảnh 2D thông thường.
