...
VIEW_POLICIES = ["random", "diversity_aware", "overlap_aware", "hybrid"]
IMAGE_SIZE = 224
MAX_POINTS = 50000
...
def main():
    ...
    # Run 4: View selection ablation.
    run4_dir = outputs_root / "run_04_view_selection"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        for policy in VIEW_POLICIES:
            vc_dir = run4_dir / policy / f"{vc}_views"
            ...
            np.savez_compressed(vc_dir / "point_clouds.npz", pred=pred, gt=gt)
            row = {
                "run": "run_04_view_selection",
                "method": f"MV-DUSt3R+ B1 + {policy}",
                "scene": SCENE,
                "num_views": vc,
                "view_policy": policy,
    ...
    write_csv(run4_dir / "metrics.csv", rows)
    best_by_view_count = []
    for vc in VIEW_COUNTS:
        vc_rows = [r for r in rows if r["num_views"] == vc]
        best_by_view_count.append(max(vc_rows, key=lambda r: r["fscore"]))
    write_csv(run4_dir / "best_by_view_count.csv", best_by_view_count)
    (run4_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run4_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_04_view_selection",
    ...
    for row in rows:
        print(row)
    print("Run 4 best by view count:")
