...
VIEW_COUNTS = [2, 3, 4, 5]
CONF_PERCENT = 3.0
BEST_CONF_BY_VIEW_COUNT = {2: 0.2, 3: 0.7, 4: 0.2, 5: 0.5}
BEST_POLICY_BY_VIEW_COUNT = {2: "hybrid", 3: "diversity_aware", 4: "hybrid", 5: "diversity_aware"}
AMBIGUITY_VARIANTS = ["A0_no_ambiguity", "A1_U_only", "A2_U_R", "A3_S_U_R", "A4_S_U_R_anchor"]
IMAGE_SIZE = 224
MAX_POINTS = 50000
F_SCORE_THRESHOLD_M = 0.05
SUPPORT_RADIUS_M = 0.05
FUSION_SAMPLE_PER_VIEW = 25000
...
def choose_views(scene_dir, view_count, policy):
    if policy == "random":
        return choose_random_views(scene_dir, view_count)
...
def output_to_view_arrays(output):
    pts = [output["pred1"]["pts3d"][0].detach().cpu().numpy()]
    pts += [x["pts3d_in_other_view"][0].detach().cpu().numpy() for x in output["pred2s"]]
    conf = [output["pred1"]["conf"][0].detach().cpu().numpy()]
    conf += [x["conf"][0].detach().cpu().numpy() for x in output["pred2s"]]
    return pts, conf
...
def ambiguity_cloud_from_output(output, variant, conf_percent):
    if variant == "A0_no_ambiguity":
        pred, threshold = output_to_pred_cloud(output, conf_percent=conf_percent)
        return pred, {
            "ambiguity_threshold": 0.0,
            "conf_threshold": threshold,
            "ambiguity_retained_ratio": 1.0,
        }

    pts_views, conf_views = output_to_view_arrays(output)
    rng = np.random.default_rng(SEED)
    sampled_pts = []
    sampled_conf = []
    for vid, (pts, conf) in enumerate(zip(pts_views, conf_views)):
        flat_pts = pts.reshape(-1, 3)
        flat_conf = conf.reshape(-1)
        valid = np.isfinite(flat_pts).all(axis=1) & np.isfinite(flat_conf)
        flat_pts = flat_pts[valid]
        flat_conf = flat_conf[valid]
        if len(flat_pts) > FUSION_SAMPLE_PER_VIEW:
            idx = rng.choice(len(flat_pts), FUSION_SAMPLE_PER_VIEW, replace=False)
            flat_pts = flat_pts[idx]
            flat_conf = flat_conf[idx]
        sampled_pts.append(flat_pts.astype(np.float32))
        sampled_conf.append(flat_conf.astype(np.float32))

    pts_all = np.concatenate(sampled_pts, axis=0)
    conf_all = np.concatenate(sampled_conf, axis=0)
    c_score = normalize01(conf_all)
    uncertainty = 1.0 - c_score

    trees = [cKDTree(p) for p in sampled_pts]
    support = np.zeros(len(pts_all), dtype=np.float32)
    offset = 0
    for vid, pts in enumerate(sampled_pts):
        n = len(pts)
        nearest_dists = []
        for oid, tree in enumerate(trees):
            if oid == vid:
                continue
            d, _ = tree.query(pts, k=1, workers=-1)
            nearest_dists.append(d.astype(np.float32))
        nearest_dists = np.stack(nearest_dists, axis=1) if nearest_dists else np.zeros((n, 1), dtype=np.float32)
        support[offset : offset + n] = (nearest_dists < SUPPORT_RADIUS_M).mean(axis=1)
        offset += n

    # Dense local neighborhoods are a weak proxy for repeated-pattern risk.
    all_tree = cKDTree(pts_all)
    k = min(8, len(pts_all))
    local_d, _ = all_tree.query(pts_all, k=k, workers=-1)
    local_d = local_d[:, 1:] if local_d.ndim == 2 and local_d.shape[1] > 1 else local_d
    repetition = np.exp(-np.mean(local_d, axis=1) / SUPPORT_RADIUS_M).astype(np.float32)

    anchor_tree = cKDTree(sampled_pts[0])
    anchor_dist, _ = anchor_tree.query(pts_all, k=1, workers=-1)
    anchor_support = np.exp(-anchor_dist / ANCHOR_RADIUS_M).astype(np.float32)

    if variant == "A1_U_only":
        ambiguity = uncertainty
    elif variant == "A2_U_R":
        ambiguity = 0.55 * uncertainty + 0.45 * repetition
    elif variant == "A3_S_U_R":
        ambiguity = 0.40 * uncertainty + 0.30 * repetition + 0.30 * (1.0 - support)
    elif variant == "A4_S_U_R_anchor":
        ambiguity = 0.35 * uncertainty + 0.25 * repetition + 0.25 * (1.0 - support) + 0.15 * (1.0 - anchor_support)
    else:
        raise ValueError(f"Unknown ambiguity variant: {variant}")

    keep = min(MAX_POINTS, len(pts_all))
    keep_idx = np.argpartition(ambiguity, keep - 1)[:keep]
    pred = pts_all[keep_idx]
    return pred.astype(np.float32), {
        "ambiguity_threshold": float(np.max(ambiguity[keep_idx])),
        "conf_threshold": 0.0,
        "ambiguity_retained_ratio": float(keep / max(1, len(pts_all))),
    }
...
def main():
    ...
    # Run 7: Repeated-structure / ambiguity filtering ablation.
    run7_dir = outputs_root / "run_07_repeated_structure_filtering"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        policy = BEST_POLICY_BY_VIEW_COUNT[vc]
        vc_dir = run7_dir / policy / f"{vc}_views"
        view_files = choose_views(scene_dir, vc, policy)
        selected_views[f"{policy}_{vc}"] = [str(p) for p in view_files]
        output, glb_file, runtime = run_inference(model, root, view_files, vc_dir)
        conf_percent = BEST_CONF_BY_VIEW_COUNT[vc]
        gt, _ = build_gt_cloud(view_files)
        for variant in AMBIGUITY_VARIANTS:
            pred, filter_info = ambiguity_cloud_from_output(output, variant, conf_percent)
            metrics = compute_metrics(pred, gt)
            np.savez_compressed(vc_dir / f"{variant}_point_clouds.npz", pred=pred, gt=gt)
            row = {
                "run": "run_07_repeated_structure_filtering",
                "method": f"MV-DUSt3R+ {policy} + {variant}",
                "scene": SCENE,
                "num_views": vc,
                "view_policy": policy,
                "ambiguity_variant": variant,
    ...
    write_csv(run7_dir / "metrics.csv", rows)
    best_by_view_count = []
    for vc in VIEW_COUNTS:
        vc_rows = [r for r in rows if r["num_views"] == vc]
        best_by_view_count.append(max(vc_rows, key=lambda r: r["fscore"]))
    write_csv(run7_dir / "best_by_view_count.csv", best_by_view_count)
    (run7_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run7_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_07_repeated_structure_filtering",
                ...
                "view_policy_by_view_count": BEST_POLICY_BY_VIEW_COUNT,
                "ambiguity_variants": AMBIGUITY_VARIANTS,
                "support_radius_m": SUPPORT_RADIUS_M,
                "fusion_sample_per_view": FUSION_SAMPLE_PER_VIEW,
                "anchor_radius_m": ANCHOR_RADIUS_M,
                "seed": SEED,
                "confidence_by_view_count": BEST_CONF_BY_VIEW_COUNT,
                "run6_takeaway": "The tested front-depth occlusion filter reduced metrics strongly, so ambiguity filtering is tested from the F0 baseline without O(p).",
    ...
    for row in rows:
        print(row)
    print("Run 7 best by view count:")
    for row in best_by_view_count:
        print(row)
