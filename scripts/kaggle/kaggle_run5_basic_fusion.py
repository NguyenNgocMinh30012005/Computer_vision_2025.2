...
FUSION_VARIANTS = ["F0_baseline", "F1_C", "F2_C_Smv", "F3_C_Smv_M"]
IMAGE_SIZE = 224
MAX_POINTS = 50000
...
def fused_cloud_from_output(output, variant, conf_percent):
    if variant == "F0_baseline":
        return output_to_pred_cloud(output, conf_percent=conf_percent)
    ...
    for vid, (pts, conf) in enumerate(zip(pts_views, conf_views)):
        ...
        sampled_view.append(np.full(len(flat_pts), vid, dtype=np.int32))

    pts_all = np.concatenate(sampled_pts, axis=0)
    conf_all = np.concatenate(sampled_conf, axis=0)
    view_all = np.concatenate(sampled_view, axis=0)
    ...
    if variant == "F1_C":
        ...
        agreement_sum = np.zeros(len(pts_all), dtype=np.float32)
        offset = 0
        for vid, pts in enumerate(sampled_pts):
            ...
            agreement_sum[offset : offset + n] = nearest_dists.mean(axis=1)
            offset += n

        s_score = support
        if variant == "F2_C_Smv":
            score = 0.65 * c_score + 0.35 * s_score
        elif variant == "F3_C_Smv_M":
            m_score = np.exp(-agreement_sum / SUPPORT_RADIUS_M).astype(np.float32)
            score = 0.50 * c_score + 0.30 * s_score + 0.20 * m_score
        else:
            raise ValueError(f"Unknown fusion variant: {variant}")

    keep = min(MAX_POINTS, len(pts_all))
    top_idx = np.argpartition(score, -keep)[-keep:]
    pred = pts_all[top_idx]
    threshold = float(np.min(score[top_idx]))
    return pred.astype(np.float32), threshold
...
def main():
    ...
    # Run 5: Basic fusion ablation on best view policy from Run 4.
    run5_dir = outputs_root / "run_05_basic_fusion"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        policy = BEST_POLICY_BY_VIEW_COUNT[vc]
        vc_dir = run5_dir / policy / f"{vc}_views"
    ...
    write_csv(run5_dir / "metrics.csv", rows)
    best_by_view_count = []
    for vc in VIEW_COUNTS:
        vc_rows = [r for r in rows if r["num_views"] == vc]
        best_by_view_count.append(max(vc_rows, key=lambda r: r["fscore"]))
    write_csv(run5_dir / "best_by_view_count.csv", best_by_view_count)
    (run5_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run5_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_05_basic_fusion",
    ...
    for row in rows:
        print(row)
    print("Run 5 best by view count:")
