...
SENSITIVITY_CONF_PERCENTS = [0.1, 0.2, 0.3, 0.5, 0.7, 1.0, 2.0, 3.0]
IMAGE_SIZE = 224
MAX_POINTS = 50000
...
def plot_conf_sensitivity(rows, out_path):
    fig, ax = plt.subplots(figsize=(7, 4.2), dpi=160)
    for vc in VIEW_COUNTS:
        vc_rows = sorted([r for r in rows if r["num_views"] == vc], key=lambda r: r["conf_percent"])
        ax.plot([r["conf_percent"] for r in vc_rows], [r["fscore"] for r in vc_rows], marker="o", label=f"{vc} views")
    ax.set_xlabel("Confidence percentile threshold")
    ax.set_ylabel("F-score")
    ax.set_title("Confidence sensitivity")
...
def plot_runtime_summary(rows, out_path):
    best_rows = []
    for vc in VIEW_COUNTS:
        vc_rows = [r for r in rows if r["num_views"] == vc]
        best_rows.append(max(vc_rows, key=lambda r: r["fscore"]))
    fig, ax1 = plt.subplots(figsize=(7, 4.2), dpi=160)
    xs = [r["num_views"] for r in best_rows]
    ax1.plot(xs, [r["fscore"] for r in best_rows], marker="o", color="#2563eb", label="F-score")
    ax1.set_xlabel("Number of views")
    ax1.set_ylabel("Best F-score", color="#2563eb")
    ax1.tick_params(axis="y", labelcolor="#2563eb")
    ax1.grid(True, alpha=0.25)
    ax2 = ax1.twinx()
    ax2.plot(xs, [r["runtime_seconds"] for r in best_rows], marker="s", color="#dc2626", label="Runtime")
    ax2.set_ylabel("Runtime per scene (s)", color="#dc2626")
    ax2.tick_params(axis="y", labelcolor="#dc2626")
    ax1.set_title("Accuracy-runtime tradeoff")
    fig.tight_layout()
    fig.savefig(out_path)
    plt.close(fig)


def plot_case_summary(out_path):
    # Values from Run 9 summary, kept here so the final report figure is reproducible from this notebook output.
    case_summary = {
        "normal": {2: 0.7377875325640727, 3: 0.7665615469992826, 4: 0.8193749600030213, 5: 0.795451862074579},
        "far_reference": {2: 0.730137755054185, 3: 0.7665615469992826, 4: 0.7636693604835393, 5: 0.795451862074579},
        "occlusion_heavy": {2: 0.6218517701663322, 3: 0.5980650335484705, 4: 0.7138388346179425, 5: 0.7535771395239254},
        "repeated_structure": {2: 0.7377875325640727, 3: 0.7847460901585933, 4: 0.8193749600030213, 5: 0.7644176755141884},
    }
    cases = list(case_summary.keys())
    data = np.array([[case_summary[c][vc] for vc in VIEW_COUNTS] for c in cases])
    fig, ax = plt.subplots(figsize=(7.5, 3.8), dpi=160)
    im = ax.imshow(data, cmap="viridis", vmin=0.55, vmax=0.86)
    ax.set_xticks(range(len(VIEW_COUNTS)), [str(v) for v in VIEW_COUNTS])
    ax.set_yticks(range(len(cases)), cases)
    ax.set_xlabel("Number of views")
    ax.set_title("Run 9 mean F-score by case")
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            ax.text(j, i, f"{data[i, j]:.3f}", ha="center", va="center", color="white", fontsize=8)
    fig.colorbar(im, ax=ax, label="Mean F-score")
    fig.tight_layout()
    fig.savefig(out_path)
    plt.close(fig)
...
def main():
    ...
    # Run 10: Sensitivity analysis and report figures.
    run10_dir = outputs_root / "run_10_sensitivity_visualization"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        best_policy = BEST_POLICY_BY_VIEW_COUNT[vc]
        best_dir = run10_dir / best_policy / f"{vc}_views"
        best_views = choose_views(scene_dir, vc, best_policy)
        selected_views[f"{best_policy}_{vc}"] = [str(p) for p in best_views]
        best_output, best_glb, best_runtime = run_inference(model, root, best_views, best_dir)
        best_gt, _ = build_gt_cloud(best_views)
        for conf_percent in SENSITIVITY_CONF_PERCENTS:
            pred, conf_threshold = output_to_pred_cloud(best_output, conf_percent=conf_percent)
            metrics = compute_metrics(pred, best_gt)
            row = {
                "run": "run_10_sensitivity_visualization",
                "method": "Full pipeline confidence sensitivity",
                "scene": SCENE,
                "num_views": vc,
                "view_policy": best_policy,
                "fusion": "F0_baseline",
                ...
                "output_glb": str(best_glb),
                **metrics,
            }
            rows.append(row)
            print("Run 10 row:", row)
        del best_output
        torch.cuda.empty_cache()

    write_csv(run10_dir / "confidence_sensitivity.csv", rows)
    best_by_view_count = []
    for vc in VIEW_COUNTS:
        vc_rows = [r for r in rows if r["num_views"] == vc]
        best_by_view_count.append(max(vc_rows, key=lambda r: r["fscore"]))
    write_csv(run10_dir / "best_sensitivity_by_view_count.csv", best_by_view_count)
    plot_conf_sensitivity(rows, run10_dir / "fig_conf_sensitivity.png")
    plot_runtime_summary(rows, run10_dir / "fig_runtime_vs_views.png")
    plot_case_summary(run10_dir / "fig_case_summary.png")
    (run10_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run10_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_10_sensitivity_visualization",
                ...
                "sensitivity_conf_percents": SENSITIVITY_CONF_PERCENTS,
                "seed": SEED,
                ...
                "full_pipeline_definition": "Best view policy + confidence tuning + F0 baseline; occlusion and ambiguity disabled because ablations were negative on the smoke scene.",
    ...
    print("Run 10 sensitivity metrics:")
    for row in rows:
        print(row)
    print("Run 10 best sensitivity by view count:")
    for row in best_by_view_count:
        print(row)
    print("Run 10 figures:")
    for name in ["fig_conf_sensitivity.png", "fig_runtime_vs_views.png", "fig_case_summary.png"]:
        print(str(run10_dir / name))
