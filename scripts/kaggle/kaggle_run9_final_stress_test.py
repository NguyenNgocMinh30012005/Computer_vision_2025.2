...
def main():
    ...
    # Run 9: Stress test the selected full pipeline across view counts and case buckets.
    run9_dir = outputs_root / "run_09_final_stress_test"
    rows = []
    selected_views = {}
    for scene_dir in scene_dirs:
        for case_type, case_policy in CASE_POLICIES.items():
            for vc in VIEW_COUNTS:
                policy = BEST_POLICY_BY_VIEW_COUNT[vc] if case_policy == "best" else case_policy
                vc_dir = run9_dir / case_type / scene_dir.name / f"{vc}_views"
                view_files = choose_views(scene_dir, vc, policy)
                selected_views[f"{case_type}_{scene_dir.name}_{vc}"] = [str(p) for p in view_files]
                ...
                np.savez_compressed(vc_dir / "full_point_clouds.npz", pred=pred, gt=gt)
                row = {
                    "run": "run_09_final_stress_test",
                    "id": "Full",
                    "method": "Best ablated pipeline stress test",
                    "case_type": case_type,
    ...
    write_csv(run9_dir / "metrics.csv", rows)
    summary_rows = []
    for case_type in CASE_POLICIES:
        for vc in VIEW_COUNTS:
            subset = [r for r in rows if r["case_type"] == case_type and r["num_views"] == vc]
            if not subset:
                continue
            row = {
                "run": "run_09_final_stress_test",
                ...
                "mean_accuracy": float(np.mean([r["accuracy"] for r in subset])),
                "mean_completeness": float(np.mean([r["completeness"] for r in subset])),
                "mean_precision": float(np.mean([r["precision"] for r in subset])),
                "mean_recall": float(np.mean([r["recall"] for r in subset])),
                "mean_fscore": float(np.mean([r["fscore"] for r in subset])),
                "mean_chamfer": float(np.mean([r["chamfer"] for r in subset])),
                "mean_runtime_seconds": float(np.mean([r["runtime_seconds"] for r in subset])),
            }
            summary_rows.append(row)
    write_csv(run9_dir / "summary_by_case_and_view.csv", summary_rows)
    (run9_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run9_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_09_final_stress_test",
                ...
                "full_pipeline_definition": "Best view policy + confidence tuning + F0 baseline; occlusion and ambiguity disabled because ablations were negative.",
    ...
    for row in rows:
        print(row)
    print("Run 9 summary by case and view:")
