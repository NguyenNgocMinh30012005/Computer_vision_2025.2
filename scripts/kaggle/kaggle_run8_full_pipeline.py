...
def main():
    ...
    # Run 8: Final comparison after ablations.
    run8_dir = outputs_root / "run_08_full_pipeline"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        random_dir = run8_dir / "random" / f"{vc}_views"
        random_views = choose_views(scene_dir, vc, "random")
        selected_views[f"random_{vc}"] = [str(p) for p in random_views]
        random_output, random_glb, random_runtime = run_inference(model, root, random_views, random_dir)
        random_gt, _ = build_gt_cloud(random_views)

        b0_pred, b0_conf_threshold = output_to_pred_cloud(random_output, conf_percent=CONF_PERCENT)
        b0_metrics = compute_metrics(b0_pred, random_gt)
        np.savez_compressed(random_dir / "B0_point_clouds.npz", pred=b0_pred, gt=random_gt)
        b0_row = {
            "run": "run_08_full_pipeline",
            "id": "B0",
            ...
            "output_glb": str(random_glb),
            **b0_metrics,
        }
        rows.append(b0_row)
        print("Run 8 row:", b0_row)

        b1_conf_percent = BEST_CONF_BY_VIEW_COUNT[vc]
        b1_pred, b1_conf_threshold = output_to_pred_cloud(random_output, conf_percent=b1_conf_percent)
        b1_metrics = compute_metrics(b1_pred, random_gt)
        np.savez_compressed(random_dir / "B1_point_clouds.npz", pred=b1_pred, gt=random_gt)
        b1_row = {
            "run": "run_08_full_pipeline",
            "id": "B1",
            "method": "MV-DUSt3R+ random views + confidence tuning",
            "scene": SCENE,
            "num_views": vc,
            "view_policy": "random",
            "fusion": "default",
            ...
            "ambiguity": "disabled",
            "runtime_seconds": random_runtime,
            "conf_percent": b1_conf_percent,
            "conf_threshold": b1_conf_threshold,
            "output_glb": str(random_glb),
            **b1_metrics,
        }
        rows.append(b1_row)
        print("Run 8 row:", b1_row)
        del random_output
        torch.cuda.empty_cache()

        best_policy = BEST_POLICY_BY_VIEW_COUNT[vc]
        best_dir = run8_dir / best_policy / f"{vc}_views"
        ...
        best_pred, best_conf_threshold = output_to_pred_cloud(best_output, conf_percent=b1_conf_percent)
        best_metrics = compute_metrics(best_pred, best_gt)
        np.savez_compressed(best_dir / "best_point_clouds.npz", pred=best_pred, gt=best_gt)

        stage_rows = [
            ("V", "B1 + best view selection", "F0_not_applied", "disabled", "disabled"),
            ("F", "V + F0 baseline fusion", "F0_baseline", "disabled", "disabled"),
            ("O", "F + occlusion disabled after failed Run 6", "F0_baseline", "disabled_after_run6", "disabled"),
            ("A", "F + ambiguity disabled after failed Run 7", "F0_baseline", "disabled", "disabled_after_run7"),
            ("Full", "Best ablated pipeline", "F0_baseline", "disabled_after_run6", "disabled_after_run7"),
        ]
        for stage_id, method, fusion, occlusion, ambiguity in stage_rows:
            row = {
                "run": "run_08_full_pipeline",
                "id": stage_id,
                "method": method,
    ...
    write_csv(run8_dir / "metrics.csv", rows)
    final_rows = [r for r in rows if r["id"] == "Full"]
    write_csv(run8_dir / "full_pipeline_by_view_count.csv", final_rows)
    (run8_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run8_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_08_full_pipeline",
    ...
    for row in rows:
        print(row)
    print("Run 8 full pipeline by view count:")
