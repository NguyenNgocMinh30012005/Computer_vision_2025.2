...
def main():
    ...
    # Run 1: evaluator smoke test on 5 random views.
    run1_dir = outputs_root / "run_01_evaluation_pipeline"
    view_files = choose_random_views(scene_dir, 5)
    output, glb_file, runtime = run_inference(model, root, view_files, run1_dir / "sample_eval")
    pred, conf_threshold = output_to_pred_cloud(output)
    gt, depth_stats = build_gt_cloud(view_files)
    metrics = compute_metrics(pred, gt)
    run1_row = {
        "run": "run_01_evaluation_pipeline",
        "method": "MV-DUSt3R+ evaluator smoke",
        "scene": SCENE,
        "num_views": 5,
    ...
    write_csv(run1_dir / "metrics.csv", [run1_row])
    (run1_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_01_evaluation_pipeline",
    ...
    np.savez_compressed(run1_dir / "sample_eval" / "point_clouds.npz", pred=pred, gt=gt)

    # Run 2: Baseline B0, random views, view counts 2/3/4/5.
    run2_dir = outputs_root / "run_02_baseline_b0"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        vc_dir = run2_dir / f"{vc}_views"
    ...
    write_csv(run2_dir / "metrics.csv", rows)
    (run2_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run2_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_02_baseline_b0",
                "dataset_slug": DATASET_SLUG,
