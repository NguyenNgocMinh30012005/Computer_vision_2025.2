...
RUN_NAME = "run_26_rsdh_v2_diagnostic_gate"
RUN19_SEED = 1919
SEED = 2626
MAX_GROUPS = int(os.environ.get("RUN26_MAX_GROUPS", "32"))
MAX_CANDIDATES_PER_GROUP = int(os.environ.get("RUN26_MAX_CANDIDATES_PER_GROUP", "5000"))
MIN_SELECTED_POINTS = int(os.environ.get("RUN26_MIN_SELECTED_POINTS", "100"))
GATE_MARGIN_F1 = float(os.environ.get("RUN26_GATE_MARGIN_F1", "0.005"))
BATCH_SIZE = 65536
PATCH_RADIUS = 12
DOWNSAMPLE = 8
...
def build_pair_features(points, xs, ys, view_ids, conf, view_files, group_row, image_h, image_w):
    ...
    for i, pt in enumerate(points):
        ...
        for tgt_idx, tgt_bundle in enumerate(bundles):
            ...
            target_counts[i] += 1.0
        if (i + 1) % 5000 == 0:
            print("Run 26 pair features:", {"candidates": i + 1, "pairs": len(features)})
    if not features:
...
def evaluate_group(rsdh, threshold, mean, std, backbone, root, scene_lookup, group_row, out_dir):
    ...
    print("Run 26 group views:", {"group": group_row.get("group_key"), "views": [p.name for p in view_files]})
    ...
    common_stats = {
        ...
        "run24_rsdh_threshold": threshold,
    ...
    conf_match_mask, conf_match_threshold = exact_topk_mask(conf, matched_ratio)
    rows.append(
        score_selection(
            points,
            conf_match_mask,
            gt,
            labels,
            "confidence_top_match_fixed_ratio",
            group_row,
            {
                **common_stats,
                "conf_percent": final_percent,
                "conf_threshold": conf_match_threshold,
                "rsdh_threshold": "",
                "score_threshold": conf_match_threshold,
    ...
    for method, score in rank_scores.items():
        mask, score_threshold = exact_topk_mask(score, matched_ratio, tie_breaker=conf)
        rows.append(
            score_selection(
                points,
                mask,
    ...
    for ratio in RANK_RATIOS:
        conf_mask, conf_score_threshold = exact_topk_mask(conf, ratio)
        rows.append(
            score_selection(
                points,
                conf_mask,
                ...
                group_row,
                {
                    **common_stats,
                    ...
                    "rsdh_threshold": "",
                    "score_threshold": conf_score_threshold,
                    "target_selected_ratio": ratio,
                },
            )
        )
        for method, score in [("rsdh_max", max_prob), ("combined_max", combined_max)]:
            mask, score_threshold = exact_topk_mask(score, ratio, tie_breaker=conf)
            rows.append(
                score_selection(
                    points,
                    mask,
...
def make_gate_decision(summary_rows):
    ...
    baselines = [r for r in val_rows if r["method_family"] == "baseline"]
    learned = [r for r in val_rows if r["method_family"] == "learned_rsdh"]
    ...
    return [
        {
            ...
            "recommendation": (
                ...
                if method_family(selected) == "learned_rsdh"
                else "Keep RSDH v2 out of reconstruction; the apparent gain is matched by non-learned candidate-retention baselines."
            ),
        }
    ]
...
def main():
    ...
    print("Run 26 eval groups:", len(eval_groups))
    for row in eval_groups:
        print("Run 26 selected group:", row)
        ...
        json.dumps(
            {
                ...
                "note": (
                    "Run 26 is a tie-controlled diagnostic gate for the Run 24 image-only RSDH v2 head. "
                    "It reruns the Run 25 integration setup, adds all-candidate and confidence top-k baselines, "
                    "uses exact top-k masks to prevent score ties from silently keeping every candidate, and gates "
                    "RSDH policies against the best non-learned candidate-retention baseline rather than only fixed confidence. "
                    "Absolute F-scores use a capped candidate pool for tractable image-patch scoring; the gate compares all methods on the same pool."
                ),
            },
            indent=2,
        )
    ...
    print("Run 26 summary:")
    for row in summary_rows:
        print(row)
    print("Run 26 gate decision:")
