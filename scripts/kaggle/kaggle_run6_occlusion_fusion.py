...
OCCLUSION_VARIANTS = ["O0_F0_baseline", "O1_front_depth_occlusion"]
IMAGE_SIZE = 224
MAX_POINTS = 50000
F_SCORE_THRESHOLD_M = 0.05
OCCLUSION_MARGIN_M = 0.05
OCCLUSION_CELL_SIZE = 2
...
def front_depth_occlusion_filter(points, image_size=IMAGE_SIZE, margin=OCCLUSION_MARGIN_M, cell=OCCLUSION_CELL_SIZE):
    """Keep points that are near the front-most depth in a coarse image cell."""
    if len(points) == 0:
        return points, {"occlusion_retained_ratio": 0.0, "occlusion_margin_m": margin, "occlusion_cell_size": cell}

    pts = points[np.isfinite(points).all(axis=1)]
    z = pts[:, 2]
    valid_z = z > 1e-6
    if valid_z.sum() < 100:
        return points, {"occlusion_retained_ratio": 1.0, "occlusion_margin_m": margin, "occlusion_cell_size": cell}

    pts_valid = pts[valid_z]
    z = pts_valid[:, 2]
    focal = image_size / (2.0 * np.tan(np.deg2rad(60.0) / 2.0))
    u = focal * (pts_valid[:, 0] / z) + image_size / 2.0
    v = focal * (pts_valid[:, 1] / z) + image_size / 2.0
    inside = (u >= 0) & (u < image_size) & (v >= 0) & (v < image_size)
    if inside.sum() < 100:
        return points, {"occlusion_retained_ratio": 1.0, "occlusion_margin_m": margin, "occlusion_cell_size": cell}

    pts_inside = pts_valid[inside]
    z_inside = z[inside]
    ui = np.floor(u[inside] / cell).astype(np.int32)
    vi = np.floor(v[inside] / cell).astype(np.int32)
    grid_w = int(np.ceil(image_size / cell))
    cell_id = vi * grid_w + ui

    order = np.argsort(cell_id)
    sorted_cell = cell_id[order]
    sorted_z = z_inside[order]
    unique_cells, start_idx = np.unique(sorted_cell, return_index=True)
    min_z = np.minimum.reduceat(sorted_z, start_idx)
    depth_map = dict(zip(unique_cells.tolist(), min_z.tolist()))
    front_z = np.array([depth_map[c] for c in cell_id], dtype=np.float32)
    keep_inside = z_inside <= front_z + margin

    filtered = pts_inside[keep_inside]
    if len(filtered) < 1000:
        return points, {"occlusion_retained_ratio": 1.0, "occlusion_margin_m": margin, "occlusion_cell_size": cell}

    return downsample(filtered.astype(np.float32), MAX_POINTS), {
        "occlusion_retained_ratio": float(len(filtered) / max(1, len(points))),
        "occlusion_margin_m": margin,
        "occlusion_cell_size": cell,
    }


def occlusion_cloud_from_output(output, variant, conf_percent):
    pred, conf_threshold = output_to_pred_cloud(output, conf_percent=conf_percent)
    info = {
        "score_threshold": conf_threshold,
        "occlusion_retained_ratio": 1.0,
        "occlusion_margin_m": 0.0,
        "occlusion_cell_size": 0,
    }
    if variant == "O0_F0_baseline":
        return pred, info
    if variant == "O1_front_depth_occlusion":
        filtered, occ_info = front_depth_occlusion_filter(pred)
        info.update(occ_info)
        return filtered, info
    raise ValueError(f"Unknown occlusion variant: {variant}")
...
def main():
    ...
    # Run 6: Occlusion-aware fusion ablation on the strongest F0 baseline from Run 5.
    run6_dir = outputs_root / "run_06_occlusion_fusion"
    rows = []
    selected_views = {}
    for vc in VIEW_COUNTS:
        policy = BEST_POLICY_BY_VIEW_COUNT[vc]
        vc_dir = run6_dir / policy / f"{vc}_views"
        ...
        for variant in OCCLUSION_VARIANTS:
            pred, filter_info = occlusion_cloud_from_output(output, variant, conf_percent)
            metrics = compute_metrics(pred, gt)
            np.savez_compressed(vc_dir / f"{variant}_point_clouds.npz", pred=pred, gt=gt)
            row = {
                "run": "run_06_occlusion_fusion",
                "method": f"MV-DUSt3R+ {policy} + {variant}",
                "scene": SCENE,
                "num_views": vc,
                "view_policy": policy,
                "occlusion_variant": variant,
    ...
    write_csv(run6_dir / "metrics.csv", rows)
    best_by_view_count = []
    for vc in VIEW_COUNTS:
        vc_rows = [r for r in rows if r["num_views"] == vc]
        best_by_view_count.append(max(vc_rows, key=lambda r: r["fscore"]))
    write_csv(run6_dir / "best_by_view_count.csv", best_by_view_count)
    (run6_dir / "selected_views.json").write_text(json.dumps(selected_views, indent=2))
    (run6_dir / "run_config.json").write_text(
        json.dumps(
            {
                "run": "run_06_occlusion_fusion",
                ...
                "occlusion_cell_size": OCCLUSION_CELL_SIZE,
                "seed": SEED,
                "confidence_by_view_count": BEST_CONF_BY_VIEW_COUNT,
                "run5_takeaway": "F1/F2/F3 custom fusion reduced metrics on the smoke scene, so Run 6 uses F0 baseline plus a conservative front-depth occlusion filter.",
    ...
    for row in rows:
        print(row)
    print("Run 6 best by view count:")
